...
<template>
  ...
  <main>
    ...
    <div class="input-group space-between">
      ...
      <input class="radio-button" type="radio" v-model="state.gender" id="female" value="female" />
      <label class="radio-button-label" for="female">Женщина</label>
    </div>
    <div class="input-group ">
      <label class="select-label" for="client-group">Группа клиентов</label>
      <select class="input-select-multiple" v-model="state.clientGroup" id="client-group" multiple>
    ...
    <div class="input-group ">
      ...
      <input class="checkbox" type="checkbox" v-model="state.noSms" id="no-sms" value="true" />
    </div>
  </main>
</template>